...
def main():
    ...
    try:
        ...
        # Enforce exact Polars schema casting for transaction_fact_df
        if "amount" in transaction_fact_df.columns:
            transaction_fact_df = transaction_fact_df.with_columns([
                pl.col("amount").cast(pl.Float64)
            ])
        if "customer_id" in transaction_fact_df.columns:
            transaction_fact_df = transaction_fact_df.with_columns([
                pl.col("customer_id").cast(pl.String)
            ])
        if "timestamp" in transaction_fact_df.columns:
            transaction_fact_df = transaction_fact_df.with_columns([
                pl.col("timestamp").cast(pl.Datetime)
            ])

        # Enforce exact Polars schema casting for customer_dimension_df
        if "tax_id" in customer_dimension_df.columns:
            customer_dimension_df = customer_dimension_df.with_columns([
                pl.col("tax_id").cast(pl.String)
            ])
